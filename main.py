...
add_project(conn , ('name', 'start_date', 'end_date'))

add_task(conn, (1, 'SQL', 'SQL task', 'TBD', 'Monday', 'Sunday'))

update(conn, 'tasks', 5, nazwa='SQL_2', status='In Progress', end_date='Saturday')

#print(delete_where(conn, 'tasks', status='End'))

#print(select_task_by_status(conn , 'In progress'))

#print(select_where(conn, 'tasks', status='In progress', projekt_id= 5, start_date='Tuesday'))
